    ...
    const T* dense_matrix, const int shape[2], const int blocksize[2])
{
    ...
    printf("shape %d %d, blocksize %d %d \n", shape[0], shape[1], blocksize[0], blocksize[1]);
    ...
    //for(int i=0; i<2; ++i){
    //    for(int j=0; i<64; ++j){
    //        printf("%d", *(int*)(dense_matrix+i*shape[1]+j));
    //    }
    //    printf("\n");
    //}
        ...
        for (int b_col = 0; b_col < bsr_matrix->shape[1] / blocksize[1]; b_col++) {
            ...
            for (int i = 0; i < bsr_matrix->blocksize[0]; i++) {
                for (int j = 0; j < bsr_matrix->blocksize[1]; j++) {
                    //printf("densestart i:%d j:%d val:%d \n", i, j, dense_start[i * shape[1] + j]);
...
done_check_zero:
    ...
    rowptr.push_back(colidxs.size());
    //for (auto i = colidxs.begin(); i != colidxs.end(); ++i) std::cout << *i << ' ';
    //printf("\n");
    //for (auto i = rowptr.begin(); i != rowptr.end(); ++i) std::cout << *i << ' ';
    //printf("\n");
